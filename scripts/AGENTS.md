...
| `test-openrouter.ts` | 使用明确提供的私有 Key 做付费 API/SDK Read 实测；不连接飞书或 daemon，支持请求参数核验与原生 resume 序列 |
...
| `sync-runtime-pins.ts` | 从 Bun 锁文件同步 DSH 包族及安全依赖的发布版本声明，随后需用 Bun 同步锁文件 |
| `check-installed-package.ts` | 在明确的 npm prefix 验收 tarball 的实际 DSH 版本、命令链接及原生目录，使用私有临时目录，不连接飞书或模型 API |
