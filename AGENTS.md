...
- API、模型目录、额度、上传和进程启动失败必须记录并向调用方显示；缺失数据用 `MISS`，不能伪造成功或偷偷更换来源。仅允许对已知瞬态错误有限重试。Codex 自动选号成功时，其他账号的缓存过期、模型不支持、查询或认证失败等诊断仅保留在日志和主动查询面板，不单独推送警告；无法启动或继续任务的失败仍须明确提示。
...
- `agent-updates.ts` 按 `[runtime.agent_auto_update]` 的 `codex`、`claude`、`dsh` 三个独立开关每 6 小时更新对应 Agent，首次检查也在 6 小时后；三项默认关闭，各自防止重叠执行，失败互不阻塞。旧布尔总开关按原值兼容转换并提示迁移，不与新表混用。`lodestar-update --agents-only` 可立即安装/更新。实际程序和 SDK 从独立版本目录加载；安装成功直接启用，新进程使用新版，已有进程保留其运行目录。Windows 不覆盖或删除使用中的 EXE/DLL；安装取消按精确 PID 终止安装器进程树，未确认退出不得清理其临时目录。文件占用有限重试，最终错误必须显示。安装或查询失败只记录更新日志并向手动更新调用方返回失败，不修改当前安装记录，不影响已有安装的启动和 `lodestar-version`；已开启自动更新的 Agent 在下一个 6 小时周期再试。旧状态中的更新错误不参与启动判断或版本展示。首次未安装、安装损坏或旧版已清掉选择记录时按实际安装状态报错，不扫描目录猜测版本。自动更新 Agent 不重启 daemon。
...
- MD 首页按 Claude Code、Codex、DeepSeek Harness 分组，底层 Agent id 仍为 `claude`、`codex`、`dsh`。模型行右侧窄按钮用单字（选、显、隐、删），补录模型、显示模型、返回和翻页等宽按钮保留完整文案；模型行之间加分隔线，文字和按钮垂直居中；只有一个 effort 档位（含原生 default）时直接应用，多个档位才进入选择；两个 DeepSeek 来源都显示为 DeepSeek。OpenRouter 默认保留腾讯 Hy4、Gemini 3.8 Flash、Meta Muse Spark 1.2、MiMo-V2.5-Pro、字节 Seed 2.1 Turbo、美团 LongCat 2.0 六项；Packy Claude 入口默认保留 MiniMax-M3、claude-opus-5、claude-fable-5-1、qwen3.8-max-0902；Codex 入口保留 kimi-k3 和 grok-4.6；其他来源默认展示接口列表。Packy 主令牌为 `packy`，第二令牌为 `packy-secondary`，`packy-codex` 复用主令牌和平台地址；按接口协议筛选各 Agent 模型；已确认兼容但目录未声明该协议的模型可通过 custom_models 显式补录，不改写上游协议声明。Codex API 来源不参与订阅登录、自动选号或订阅额度查询。Packy 真实余额用独立系统访问令牌和用户 ID 查询管理端 `/api/user/self`，按站点单位换算；Codex 共用主账号余额，第二令牌可显式引用同一余额账号，推理 Key 保持独立。系统令牌不传入 Agent；hi 合并同一余额账号，footer 共用缓存，失败显示 MISS。接口项用隐藏/显示，列表外记录用补录/删除；所有来源支持 `custom_models`，补录后可选请求档位并直接使用，不能因目录未收录而清空 effort 或禁止选择。隐藏不改运行模型，删除补录项须保护正在选用的会话并清理悬空配置。footer 固定为 `agent · 模型名/effort`；所有来源的交互只读缓存，模型目录每 5 分钟、额度每 60 秒在后台刷新，启动首次预热可等待；额度按账号和平台共享缓存及并发请求；Codex 额度查询的瞬态连接错误先在同一共享请求内最多尝试 3 次（间隔 1s、4s），真实限频与认证失败不立即重试；最终失败冷却依次为 1、2、4、5 分钟，遵守上游更长 Retry-After；回复 footer 最终刷新失败可按原格式直接展示同账号成功缓存，不加缓存或刷新失败标注，无缓存或认证失败仍 MISS；主动额度面板同样只读缓存，诊断保留在后台日志和账号状态；窗口额度保留原紧凑倒计时格式 `4.1h·7%·[6.9d·17%]`，余额显示 `余额 $…` / `余额 ¥…`。OpenRouter 用 `/credits` 查账户余额，以实际响应判断权限。
...
- `workspace.ts` 统一按实际工作目录（含软链接归一化）解析目录配置；BTW/FK 与来源共享目录，WT 是独立目录。文件交付开关、项目工具/MCP 配置按目录共享；任务清单属于项目级，主群、所有 WT 及其 BTW/FK 临时群共用同一清单。模型/账号仅创建临时会话时继承快照，后续各会话独立。配置别名冲突必须报错。
- `worktree.ts` 管理 `work/*` 分支和同级 `<project>[name]` 工作区。飞书任务清单按主项目名绑定，所有 worktree 共用；只提供绑定与删除，不启动自动规划、执行、审核或合并 worker。
- `[[send: /abs/path]]` 默认走原生聊天附件及其 30 MB 上限；`files on/off` 按工作目录开启/关闭云空间（BTW/FK 共享，WT 独立），`files` 查看设置。主 Agent 启动时按工作目录设置动态注入交付标记，只有附件模式附加大小约束；文件交付不提供独立 Skill，不让 Agent 查接口或配置。`group-file-delivery.ts` 将目录开关与群文件夹绑定分开保存；v1 旧开关按已知群目录迁移，冲突需用户用 files on/off 明确统一，未解析的旧群保留。云端文件夹仍按 chat_id 持久绑定唯一目录，继承云空间模式的新群在首次交付时建自己的文件夹，目录名与实时群名一致，跨轮次和重启复用；关闭保留目录与文件。同名群不能混用目录，失效绑定不得自动重建空目录。云空间卡只列本轮文件，管理按钮打开本群全部历史交付文件的原生飞书目录；开启者和交付发起人获 full_access、群获 view，不转移所有权或开放公网分享。超过 20 MB 分片上传，限额由上游判断，失败不自动切换通道。原附件/图片通道保留，旧聊天附件不删除、不自动迁移。交付方式在本轮输入交给 Agent 时固定，开关改变从下一轮用户任务更新提示词，不重复普通输入或重启进程；记录由 paths.ts 定义，远端文件不自动清理，模板不读取文件。
