...
账号命令统一回复简洁卡片：长错误可展开查看；`codex-accounts` 仅展示主额度，进度条为六格，每页显示 4 个记录，同时标出当前模式下的顺序和 `份/h` 分数，后续页用 `codex-accounts 2` 等命令查看。列表按账号身份去重，未知身份或查询失败显示 `MISS`。`hi` 与回复 footer 只显示实际使用账号的额度；footer 保留 `4.1h·7%·[6.9d·17%]` 紧凑格式，自动换号后跟随新账号。重置卡次数只出现在账号明细和 `hi` 中。
