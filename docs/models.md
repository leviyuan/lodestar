...
GLM Key 可在首次安装时跳过，之后用群命令添加或更换。以下命令先返回回执，后台向对应平台校验凭据，通过后才保存；认证、网络或响应错误均保留原配置并给出原因。校验无需调用付费模型，也不要求先安装对应 Agent。
...
凭据校验通过后会明确返回“配置已保存”，模型目录和额度随后在后台加载；若 Agent 未安装或目录刷新失败，`md` 显示具体错误。处理后等待后台刷新即可。认证失败会提示核对 Key，网络失败会提示检查连接和代理，不会将错误正文误报为“缺少 data 数组”。

## 选择与管理模型

发送 `model`（`md`），在 Claude Code、Codex、DeepSeek Harness 分组下选择账号，再选择模型。面板直接使用缓存，不等待平台接口或 Codex 账号查询。有多个推理档位（effort）时继续选择档位，只有一个档位时直接应用。

模型目录每 5 分钟、额度每 60 秒独立在后台刷新，只有 daemon 启动会等待首次加载。`md`、`hi`、账号列表、Agent 身份目录、自动选号和回复页脚都直接读取已提交缓存；打开面板不会启动额外查询，某个来源变慢也不会占住交互。Codex 邮箱复用后台原生账号查询的结果。

同一来源和账号的并发刷新会合并，刷新完成后一次更新缓存。网络或限流失败保留成功快照，错误写入日志，目录刷新错误也会显示在模型面板；失败按 1、2、4、5 分钟退避并遵守平台更长的限流冷却。没有缓存或认证已失效时立即显示加载中或 `MISS`。修改凭据会隔离并清除受影响账号的缓存，旧请求结果不能覆盖新配置；其他账号继续可用。
...
Codex 启动和换号优先使用最近一次成功的额度缓存；能选出账号就直接启动，不等待其他账号刷新；没有可用缓存时明确显示 MISS，后台继续刷新。其他账号未参与选号的诊断保留在日志和 `codex-accounts` 中，不主动发送“部分账号 MISS”警告。只有无法启动或继续任务时，才主动展示相应失败或等待状态。
...
订阅额度通过原生 `/usage` 控制接口读取，无需发送模型对话。`hi` 展示 5 小时、总周额度和接口返回的模型专属周额度及重置倒计时。回复底部同时显示 5 小时额度与周额度：本轮所选模型有专属周额度时显示该模型的额度，否则显示总周额度，沿用紧凑格式。后台瞬态查询失败保留成功缓存；没有缓存、认证失效或专属窗口缺数值时显示 `MISS`，不将缺失的专属额度改成总额度。此 SDK 查询接口仍属实验接口，版本不支持或接口变化时明确报错。
