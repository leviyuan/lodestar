...
- Card Kit 实测 `300308 + Server Internal Error` 在本 API 域按瞬态故障有限重试；其他业务拒绝不因相近错误码扩大重试。POST 曾返回不确定结果、后续明确 Duplicate ID 时，才用一次 checked PUT 对账，确认成功再计数；未知重复 ID 不自动覆盖。页脚计时与摘要各自最多一条在途写入并合并最新内容，计时写入重试耗尽后暂停本卡自动计时，但真实状态变化、正文与终态仍可写；同轮相同上游根因合并告警，完整错误逐请求记日志。
