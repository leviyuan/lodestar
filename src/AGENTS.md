...
- Codex 模型发现的 `account/read`、`model/list` 遇到已知网络瞬态错误时重试同一接口，最多 3 次，间隔 1s、4s；额度 API 单次失败进入共享冷却，不立即重试。认证错误和无效/空模型响应直接报错，最终失败保留原始错误并显示实际尝试次数。自动选号区分账号检查失败（MISS）、未配置账号和不符合条件，不能将额度查询失败笼统显示为没有可用账号。
