...
- API Key 配置命令由 factory 的 `setup.validate` 查询账号接口，校验与保存共用配置队列；校验失败不落盘、不覆盖旧 Key、不重建目录。凭据校验不依赖 Agent 安装；保存后的运行时加载失败须明确说明“已保存”。GLM 安装向导仍可跳过，错误提示保留上游诊断并脱敏。
...
- 手动指定最高优先：`hi 备注` 通过 Session 生命周期只覆盖这次启动，已有其他账号则重启并 resume；`codex-account` 持久指定、`codex-auto` 清除。手动路径不读额度、不检查套餐、Ultra 门槛或本地模型目录；保留账号存在性、认证文件隔离、原生进程退出与登录写入互斥这些结构约束，真实错误交由 Codex 返回。自动路径仍检查模型与全部实际限额、身份、登录租约。会话启动及换号优先用最近一次成功额度缓存，能选出账号就不查询额度，也不等其他账号补缓存；没有成功缓存时才按共享冷却读取。网络刷新失败保留成功缓存，实时调度保留失败原因而展示沿用成功数据；footer 刷新失败、进程已退出或换号时可展示本条回复所属账号的成功缓存，沿用原额度格式，不加缓存或刷新失败标注。无成功缓存或认证失败仍 MISS；重新登录、删除账号与额度重置清空缓存并使已捕获的 footer 缓存读取器失效，迟到响应不得跨账号或恢复旧缓存。旧缓存不得清除原生耗尽记录，主动查额度、耗尽等待轮询、hi 和页脚均遵守同一账号缓存与冷却。运行中耗尽后转自动选择，手动原生默认模型在恢复时必须原样传给新进程，不能变成另一账号的默认模型。
...
- `codex-accounts` 等待已有全局模型刷新，按当前使用或指定的账号读取目录；缺少模型时主动刷新，仍缺失则展示真实账号与额度，并明确显示调度顺序 MISS 及具体原因。已有额外账号记录不能因默认账号后来退出登录而无法查看，也不能借用其他账号的模型或伪造调度排名。
- Codex 自动选号按账号缓存成功目录中的模型/档位不匹配结果 5 分钟，兼容新模型按账号分批开放；缓存命中不续期，到期后下次选号才重新查询同一账号，`md` 可立即刷新。一次选号的缓存和实时额度阶段共用检查，并发检查共用刷新。须区分缺模型、缺档位和明确禁用原因；查询失败保留原始错误，不缓存为尚未开放，不借用其他账号目录或旧数据放行。
...
- `usage-cache.ts` 统一额度读取的 60 秒缓存、并发合并和失败冷却（1、2、4、5 分钟，遵守更长 Retry-After）；缓存命中不续期，无定时刷新。Codex 独立查询与现有连接共用同一入口；登录、删除与重置使迟到响应失效。HTTP 来源按平台及凭据哈希隔离；`readStale` 在瞬态失败时保留最近成功值，认证失败仍返回 MISS。
