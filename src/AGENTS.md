...
- Codex 模型发现的 `account/read`、`model/list` 遇到已知网络瞬态错误时重试同一接口，最多 3 次，间隔 1s、4s；额度查询的瞬态连接错误也在同一共享请求内最多尝试 3 次（间隔 1s、4s），最终失败才进入共享冷却；真实限频、认证失败及无效响应不立即重试。认证错误和无效/空模型响应直接报错，最终失败保留原始错误并显示实际尝试次数。自动选号区分账号检查失败（MISS）、未配置账号和不符合条件，不能将额度查询失败笼统显示为没有可用账号。
