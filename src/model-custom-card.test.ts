...
`)
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (path === '/cards/id_convert') {
      if (rejectIdConvert) {
        return new Response(JSON.stringify({ code: 300121, msg: 'id convert rejected', error: { log_id: 'convert-log' } }), {
          headers: { 'Content-Type': 'application/json' },
        })
      }
    ...
    if (init?.method === 'PUT' && /\/cards\/[^/]+\/elements\//.test(path)) {
      ...
      if (rejectElementPatch) {
        return new Response(JSON.stringify({ code: 300121, msg: 'model panel rejected', error: { log_id: 'model-panel-log' } }), {
          headers: { 'Content-Type': 'application/json' },
        })
      }
    }
  ...
  test('isolated model custom card flow', async () => {
    ...
    expect(rawFallbacks.at(-1)).toContain('code=300121 message=model panel rejected log_id=model-panel-log')
    ...
    expect(rawFallbacks.at(-1)).toContain('模型 GLM-5.2 已在列表中')
    expect(rawFallbacks.at(-1)).toContain('重新发送 model')
    expect(rawFallbacks.at(-1)).toContain('code=300121 message=id convert rejected log_id=convert-log')
