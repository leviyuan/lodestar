...
- `codex-accounts` 去掉记录数和调度汇总行，按调度顺序排序后分页；每项标题为 `#1·备注「0.111/h」 · 重置 N`，重置次数取该账号额度缓存，保留零、缺失用 MISS；临期直接优先时用「临期优先」，无排名或分数用 MISS。正文首行为“邮箱：…”，随后周、可选 5h 各一行，以六格进度条、已用百分比和「5.2d」真实倒计时显示，不附加套餐或重置卡数；完整命令区保持默认折叠。调用方只读取账号邮箱缓存；后台模型/额度查询复用原生 `account/read` 发布邮箱，被额度去重的记录也保留各自账号信息。邮箱、额度及调度失败保留 MISS 和可展开的错误，不用备注或旧登录记录代替。
